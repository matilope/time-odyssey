...
@section('content')
  ...
  <form class="form-custom" action="{{ route('blogs.edit.post', ['id' => $blog->id]) }}" method="POST"
    ...
    <div class="form-div">
      ...
      <textarea class="border border-gray-900/25" name="description" id="description" rows="3"
        @error('description')
          ...
          aria-invalid="true"
        @enderror>{{ old('description', $blog->description) }}</textarea>
      @error('description')
        <div class="bg-red-100 border border-red-400 text-red-700 mt-3 px-2 py-2 rounded relative" role="alert">
    ...
    <div class="double-column-img">
      ...
      <img id="img-preview"
        src="@if ($blog->image && Storage::has($blog->image)) {{ asset('storage/' . $blog->image) }}@else {{ asset('/images/default.png') }} @endif"
    ...
    <div class="form-div">
      ...
      <textarea class="border border-gray-900/25" rows="3" name="image_description" id="image_description"
        @error('image_description')
          ...
          aria-invalid="true"
        @enderror>{{ old('image_description', $blog->image_description) }}</textarea>
      @error('image_description')
        <div class="bg-red-100 border border-red-400 text-red-700 mt-3 px-2 py-2 rounded relative" role="alert">
    ...
    <div class="form-div">
      ...
      <textarea class="border border-gray-900/25" rows="8" name="synopsis" id="synopsis"
        @error('synopsis')
          ...
          aria-invalid="true"
        @enderror>{{ old('synopsis', $blog->synopsis) }}</textarea>
      @error('synopsis')
        <div class="bg-red-100 border border-red-400 text-red-700 mt-3 px-2 py-2 rounded relative" role="alert">
