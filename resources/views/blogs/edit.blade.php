...
@section('content')
  ...
  <form class="form-custom" action="{{route('blogs.edit.post', ['id' => $blog->id])}}" method="POST" enctype="multipart/form-data">
    @csrf
    <div class="double-column">
      ...
      <div class="form-div">
        ...
        <select class="border border-gray-900/25" id="category_id" name="category_id" @error('category_id') aria-describedby="error-category" aria-invalid="true" @enderror>
    ...
    <div class="double-column-img">
      ...
      <img id="img-preview" src="@if($blog->image && Storage::has($blog->image)){{ asset('storage/' . $blog->image) }}@else {{asset('/images/default.png')}} @endif" alt="@if($blog->image_description){{$blog->image_description}}@else{{$blog->title}}@endif" />
    </div>
    <div class="form-div">
      ...
      <textarea 
        ...
        @error('image_description')
          ...
          aria-invalid="true"
        @enderror 
      >{{ old('image_description', $blog->image_description) }}</textarea>
      @error('image_description')
      <div class="bg-red-100 border border-red-400 text-red-700 mt-3 px-2 py-2 rounded relative" role="alert">
    ...
    <button type="submit" class="admin-btn edit-btn">Editar artículo</button>
  </form>
  <script>
