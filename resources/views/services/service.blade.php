...
@section('meta')
  ...
  <meta property="og:image"
    content="@if ($service->image) {{ asset('storage/' . $service->image) }} @else {{ asset('/images/banner.jpg') }} @endif" />
        ...
        <div class="lg:flex justify-center w-full sm:w-[50%]">
          <div class="lg:order-2 lg:ml-5">
            <div class="max-w-xl overflow-hidden rounded-lg mb-3">
              <img class="h-full w-auto sm:w-full mx-auto object-contain"
                src="@if ($service->image) {{ asset('storage/' . $service->image) }} @else {{ asset('/images/banner.jpg') }} @endif"
                alt="{{ $service->destiny->name }}" />
            </div>
          </div>
        </div>
          ...
          <div class="my-5">
            <div class="accordion bg-gray-50 py-1 px-2 rounded-md border-t-2 border-b-2 border-solid border-white">
              <button
                ...
                <svg class="hidden w-4 h-4 svg-plus" xmlns="http://www.w3.org/2000/svg" width="24" height="24"
                  viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round"
                  stroke-linejoin="round">
                  <path d="M5 12h14" />
                </svg>
          ...
          <div
            ...
            @auth
              <button data-id="{{ $service->id }}" data-name="{{ $service->destiny->name }}" type="button"
                class="inline-flex items-center justify-center rounded-md border-2 border-transparent bg-yellow-500 bg-none px-12 py-3 text-center text-base font-bold text-white transition-all duration-200 ease-in-out focus:shadow hover:bg-yellow-600">Contratar</button>
            @endauth
            @if (!Auth::user())
      ...
      <div class="w-full">
        <div class="border-b border-gray-300">
          <nav class="flex gap-4">
            <span
              class="border-b-2 border-yellow-500 py-4 text-sm font-medium text-gray-900 hover:border-gray-400 hover:text-gray-800">Descripción</span>
          </nav>
        </div>
